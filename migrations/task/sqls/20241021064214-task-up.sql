...
-- 5-1. 新增：請在 `COURSE_BOOKING` 新增兩筆資料：
    ...
    (
        (SELECT id FROM "USER" WHERE e-mail = 'wXlTq@hexschooltest.io'),
...
-- 5-5. 修改：`王小明` 現在已經加入直播室了，請在`COURSE_BOOKING`更新該筆預約資料（請注意，不要更新到已經取消的紀錄）：
    ...
    WHERE user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
    AND course_id = (SELECT id FROM "COURSE" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io'))
    AND booking_at = '2024-11-24 17:10:25';
...
-- 5-8. [挑戰題] 查詢：請在一次查詢中，計算用戶王小明的剩餘可用堂數，顯示須包含以下欄位： user_id , remaining_credit
    ...
    SELECT "MING_PURCHASED".user_id, ("MING_PURCHASED".total_credit - "MING_USED".used_credit) AS remaining_credit
    FROM (SELECT user_id, SUM(purchased_credits) AS total_credit FROM "CREDIT_PURCHASE" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') GROUP BY user_id) AS "MING_PURCHASED"
    INNER JOIN (SELECT user_id, COUNT(*) AS used_credit FROM "COURSE_BOOKING" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') AND status != '課程已取消' GROUP BY user_id) AS "MING_USED"
    ON "MING_PURCHASED".user_id = "MING_USED".user_id;
